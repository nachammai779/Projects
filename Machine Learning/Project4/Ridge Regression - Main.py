...
def main():
    ...
    optimal_lamda = reg_param(x_train, y_train)
    w_reg = ridge_reg(x_train, y_train, regularizer=optimal_lamda) 
...
def reg_param(x_train, y_train):
    ...
    for i, r in enumerate(regularizers):
        w = ridge_reg(x_train, y_train, regularizer=r) 
        ein[i] = calculate_error(w, x_train, y_train)
        cv_error[i] = cv(x_train, y_train, 3, r)

    print_cv_results(regularizers, ein, cv_error)
    
    return regularizers[np.argmin(cv_error)]


def print_cv_results(regularizers, ein, cv_error):
...
def cv(x_train, y_train, num_splits, regularizer):
    ...
    for i, this_d in enumerate(np.split(d, num_splits)):
        ...
        w = ridge_reg(x_train, y_train, regularizer)
...
def ridge_reg(x_train, y_train, regularizer):
